...
const (
	...
	RESOURCES_BY_TYPE  = `SELECT * FROM resources WHERE is_valid = TRUE AND type_id = ?`
	RESOURCES_BY_TOPIC  = `SELECT * FROM resources WHERE is_valid = TRUE AND topic_id = ?`
	...
	RANDOM_TOPIC  = `SELECT * FROM topics WHERE is_active = TRUE ORDER BY RAND() LIMIT 1`
	RANDOM_TOPIC_BY_CATEGORY  = `
		SELECT * FROM topics WHERE is_active = TRUE AND category_id = ? ORDER BY RAND() LIMIT 1`
	RANDOM_TOPIC_BY_DIFFICULTY  = `
		SELECT * FROM topics WHERE is_active = TRUE AND difficulty_id = ? ORDER BY RAND() LIMIT 1`
	RANDOM_TOPIC_BY_CATEGORY_AND_DIFFICULTY  = `
		SELECT * FROM topics WHERE is_active = TRUE AND category_id = ? 
		AND difficulty_id = ? ORDER BY RAND() LIMIT 1`
	...
	RANDOM_TOPIC_WITH_DETAILS = TOPIC_WITH_DETAILS_TEMPLATE + `
		ORDER BY RAND()
		LIMIT 1`
	RANDOM_TOPIC_WITH_DETAILS_BY_CATEGORY = TOPIC_WITH_DETAILS_TEMPLATE + `
		AND c.id = ?
		ORDER BY RAND()
		LIMIT 1`
	RANDOM_TOPIC_WITH_DETAILS_BY_DIFFICULTY = TOPIC_WITH_DETAILS_TEMPLATE + `
		AND d.id = ?
		ORDER BY RAND()
		LIMIT 1`
	RANDOM_TOPIC_WITH_DETAILS_BY_CATEGORY_AND_DIFFICULTY = TOPIC_WITH_DETAILS_TEMPLATE + `
		AND c.id = ?
		AND d.id = ?
		ORDER BY RAND()
		LIMIT 1`
	...
	RESOURCES_WITH_DETAILS_BY_TYPE = RESOURCE_WITH_DETAILS_TEMPLATE + ` AND rt.id = ?`
	RESOURCES_WITH_DETAILS_BY_TOPIC = RESOURCE_WITH_DETAILS_TEMPLATE + ` AND t.id = ?`
)
